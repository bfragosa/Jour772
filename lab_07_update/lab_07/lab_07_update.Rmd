...
**Q2** Which Maryland counties have a median age of at least 50? You can use the dataframe created in Q1 to answer this, but you must write code to display only those counties with a median age of at least 50. **Talbot County and Worcester County**
...
```{r}
total_calls <- montgomery_2022_overdoses %>% 
  group_by(zip) %>% 
...
```{r}
md18 <- get_acs(geography = 'zcta',
              variables = c(pop18 = 'B09021_001'),
              year = 2022) %>% 
...
```{r}
Montgomery_with_acs <- montgomery_2022_overdoses %>% 
left_join(md18, by = c('zip'='GEOID'))
```
...
Montgomery_with_acs %>% 
  ...
  summarise(total_calls=n()) %>% 
  mutate(pct_calls = (total_calls/estimate)* 1000) %>% 
...
**20889 had the highest rate of calls and 20707 had the lowest.**
...
**Write your answer here: There could be a story with the 20707 zip code. Not really 20889 because the population is so little already, the calls per 1000 people skews the percentage to be way higher than the others because of how small the population is. For the 20707 zip code, I saw that 12% of the population are moving into units since 2021, a 16% decrease since 2018-2020 which was at 28%.I could look into why there has been such a drop, whats stopping people from moving, and so on.