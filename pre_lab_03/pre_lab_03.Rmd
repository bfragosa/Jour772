...
**Task** Run the following codeblock to glimpse the data. What data type is the "Vendor Name" field? What data type is the "Amount" field? What data type is the "Date" column? Answer below. **the vendor name field is character data or chr and the amount field is number data or dbl. Date would be dbl data.**
...
**Task** Run the following codeblock to group by the vendor's zip code and show any that are less than five characters in length. How many zip codes do not have five characters? What do you think explains those records? **582, its because its an extension of the 5 digit zip code, and the four is the postal code.**
...
**Task** Run the following codeblock and describe below what each line is doing. Looking at the results, what jumps out at you as potentially problematic? **There's a bunch of missing data throughout the data set. There's also no loan data for 2009.**
...
**Task** Run the following code to arrange the grants & loan data so that the smallest amount is first. Describe what you see. **The first two columns are outliers that are less than $100**
...
**Task** Run the following codeblock to use the `clean_names()` function from janitor to standardize column names. How does it change the headers? Answer below. **It changes the headers to be lowercase and includes underscores instead of spaces between words. This makes coding alot easier because all headers are in the same format.**
...
**Task** Run the following codeblock to use str_to_upper() to make the source, grantee and description columns upper-cased. Describe what each line is doing. **Its capitalizing the data output names so that they all have consistent capitalization.**
...
**Task** Run the following codeblock to check for duplicate rows using get_dupes(). How many duplicates are possible? **We have up to 58 duplicates. But we can further filter out the data to see if these are actually duplicates.**
...
**Task** Run the following codeblock to use distinct() to get rid of duplicate rows. How many rows does the new dataframe have? Answer below. **The new dataframe has 19,453 rows.**
...
**Task** Run the following codeblock to use str_sub() to convert the ZIP codes that have nine digits to five digits, creating a new column. Look at the difference in the result - what changed? **It created a new column called zip5 and only shows the first 5 numbers in the zip codes and removed the one last four digits when zip codes had.**
...
**Task** Write and run code to answer our original question: which zip code has gotten the most amount of money from the Maryland Tourism Board? Where is that zip code, and does that zip code make sense to you as the top recipient? Write a sentence describing the top result. **The zip code that has gotten the most amount of money from the Maryland Tourism Board was 21202. This zip code is in Baltimore City and this does make sense because this is the largest city in Maryland.**
