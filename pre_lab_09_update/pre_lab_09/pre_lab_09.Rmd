...
**https://datawrapper.dwcdn.net/s5iFB/1/** Copy the url of the graphic you published here. It will begin with <https://datawrapper.dwcdn.net/>. Don't copy the browser's current url.
...
md_gov_county <- md_gov_county %>% 
  mutate(total = rowSums(across(3:8)),
...
```{r}
candidate_totals <- md_gov_county %>% 
...
**Task** Look for "Crop to data" under Appearance, and click the slider icon to enable that feature. You should see a map zoomed into Maryland with some counties in various colors. **We changed the map from USA counties to Maryland Counties so no changes were made when we cropped to data.
...
**I did not have to make too many changes to my map besides adding the texts. When I opened tooltips, it was already correctly formatted to show county and the percentage of votes Wes Moore had received. It initially said " %>% region %>%"  which gave me the county anyways, but I changed it to the "county" option to be on the safe side.** Describe the changes from the previous map
...
**https://datawrapper.dwcdn.net/GLHSA/1/** Put the URL of your map here.
